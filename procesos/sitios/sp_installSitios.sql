delimiter //
create procedure sp_installSitios(
	in v_id_sitio int,
	in v_estatus int,
	...
	if v_id_sitio = 0 and exists (select 1 from sitios where descripcion = v_descripcion) then
		set variable = 1;
	elseif v_id_sitio = 0 and not exists (select 1 from sitios where descripcion = v_descripcion) then
	...
	elseif not exists (select 1 from sitios where id_sitio = v_id_sitio) then
		set variable = 2;
	elseif exists (select 1 from sitios where not id_sitio = v_id_sitio and descripcion = v_descripcion) then
		set variable = 1;
	elseif exists (select 1 from sitios where id_sitio = v_id_sitio) then
		update sitios
